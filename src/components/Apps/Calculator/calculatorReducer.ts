...
function getMathResult({
  ...
  return Number(
    performJSMathResult({ first, operator, second }).toFixed(fractionDigits)
  );
}
...
export function calculatorReducer(state: IState, action: ActionT): IState {
  ...
  const isOperatorPressed = [Mode.OperatorPressed, Mode.ShowingResult].includes(
    mode
  );
  ...
  function getInsertedNumberResult(digit: DigitT) {
    ...
    const isDecimalNumberThatEndsWithDot =
      isDecimalMode && !isDecimal(existingNumber);
  ...
  if (isOperator(payload)) {
    ...
    const builtNumberResult = result.endsWith(".")
